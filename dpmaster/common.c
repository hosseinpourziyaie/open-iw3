...
static volatile sig_atomic_t must_open_log = true;
...
====================
*/
void Com_SetConsoleTitle (int nb_servers)
{
	char TitleText[200];
	sprintf(TitleText ,"%s - Servers in Memory : %u" , WindowTitle , nb_servers);


	/*int lenA = lstrlenA(TitleText);
	int lenW = ::MultiByteToWideChar(CP_ACP, 0, TitleText, lenA, NULL, 0);
	if (lenW>0)
	{
	LPWSTR output;
	::MultiByteToWideChar(CP_ACP, 0, TitleText, lenA, output, lenW);
	} */

	/*wchar_t wtext[20];
	mbstowcs(wtext, TitleText, strlen(TitleText)+1);//Plus null
	LPWSTR ptr = wtext;*/
	SetConsoleTitleA(TitleText);
