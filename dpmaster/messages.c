...
// Q3: "heartbeat QuakeArena-1\x0A"
// DP: "heartbeat DarkPlaces\x0A"
#define S2M_HEARTBEATEXT "heartbeatExt "
...
#define C2M_GETSERVERS "getservers "// was "getservers " //was "loadservers "

// DP: "getserversExt DarkPlaces-Quake 3 empty full ipv4 ipv6"
// IOQuake3: "getserversExt 68 empty ipv6"
#define C2M_GETSERVERSEXT "getserversExt "// was "getserversExt " //was "loadserversExt "

// Q3 & DP & QFusion:
// "getserversResponse\\...(6 bytes)...\\...(6 bytes)...\\EOT\0\0\0"
#define M2C_GETSERVERSREPONSE "getserversResponse"// was "runmenuscript" //was "getserversResponse"

// DP & IOQuake3:
// "getserversExtResponse\\...(6 bytes)...//...(18 bytes)...\\EOT\0\0\0"
#define M2C_GETSERVERSEXTREPONSE "getserversExtResponse"// was "runmenuExtscript" //was "getserversExtResponse"
...
====================
*/
static void HandleHeartbeat (const char* msg, const struct sockaddr_storage* addr, socklen_t addrlen, socket_t recv_socket, qboolean extended)
{
	char tag [64];
	...
	{
		sscanf (msg, "%63s %hu", tag, &altPort);

		Com_Printf (MSG_NORMAL, "> %s ---> heartbeatExt (%s, %i)\n",
								 peer_address, tag, altPort);
	}
	...
	if (extended)
	{
		server->user.altPort = altPort;
	}
	else
	{
		server->user.altPort = 0;
	}
...
static void HandleGetServers (const char* msg, const struct sockaddr_storage* addr, socklen_t addrlen, socket_t recv_socket, qboolean extended_request)
{
	...
	for (sv = Sv_GetFirst (); sv != NULL; sv = Sv_GetNext ())
	{
		...
		if (sv->user.address.ss_family == AF_INET)
		{
			...
			if (sv->user.altPort != 0)
			{
				packet[packetind + 7] = sv->user.altPort >> 8;
				packet[packetind + 8] = sv->user.altPort & 0xFF;	
...
static void HandleInfoResponse (server_t* server, const char* msg)
{
	...
	if (value == NULL && 0) //[NOTE] Disabled for now because IW3 doenst have clients var in infostring
		                    // TODO: Hook IW3 Server Dvar_InfoString_Big to Include clients count key\\value
	{
		Com_Printf (MSG_WARNING,
					...
					socket_t recv_socket)
{
	...
	//printf(msg);
	//printf("\n IranRev \n");
	Com_Printf (MSG_NORMAL, "> Packet From %s ---> %s\n", peer_address , msg);
	...
	else if (!strncmp (S2M_HEARTBEATEXT, msg, strlen (S2M_HEARTBEATEXT)))
	{
		HandleHeartbeat (msg + strlen (S2M_HEARTBEATEXT), address, addrlen,
						 recv_socket, true);
	}
